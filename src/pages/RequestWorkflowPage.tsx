...
export function RequestWorkflowPage() {
  ...
  const validateCurrentStep = () => {
    ...
    if (currentStep === 2 && hasSpecificInputs) {
      return detail.requiredInputs.every(input => formData.dynamicFields[input]?.trim() !== '');
    }
    return true;
  };
  ...
  const handleSubmit = () => {
    // Generate a simulated ID from the fixture format
    const newId = `REQ-${Math.floor(2400 + Math.random() * 100)}`;
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full p-2.5 bg-white border rounded-button focus:outline-none focus:ring-2 focus:ring-secondary/20 transition-all ${
                      showValidation && !formData.title.trim() ? 'border-danger' : 'border-border-strong hover:border-text-muted'
                    }`}
                    placeholder="Briefly summarize your request"
                  />
                  {showValidation && !formData.title.trim() && (
                    <p className="text-xs text-danger mt-1.5 font-medium">Required to route this request.</p>
                  )}
                </div>
