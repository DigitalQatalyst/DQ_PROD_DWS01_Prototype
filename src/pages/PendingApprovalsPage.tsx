...
export function PendingApprovalsPage() {
  ...
  return <RolePageScaffold eyebrow="Team Execution" title="Pending Approvals" purpose="Review and decide on pending requests and task closures.">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {mockApprovals.map((approval) => <div key={approval.id} className="bg-white rounded-[12px] border border-[#D8D9E6] p-5 shadow-sm">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => toast.success('Changes requested.')} className="flex-1 py-2 bg-white border border-[#D8D9E6] text-[#111118] text-sm font-medium rounded-[8px] hover:bg-[#F6F6FB] transition-colors">
                Request Changes
              </button>
            </div>
          </div>)}
