  ...
  const dropdownPanel = <div ref={dropdownRef} className="fixed right-6 top-[76px] w-[320px] bg-white rounded-2xl shadow-xl border border-border-default overflow-hidden z-[120]">
      ...
      <div className="px-3 py-4 border-b border-border-subtle">
        ...
        <div className="space-y-1">
          ...
          return <button key={role} onClick={() => setActiveRole(role)} className={`w-full flex items-center justify-between gap-3 rounded-xl px-3 py-2.5 text-left text-sm transition-colors ${isActive ? 'bg-navy-50 text-primary font-semibold' : 'text-text-secondary hover:bg-surface hover:text-primary'}`}>
                <span>{role}</span>
