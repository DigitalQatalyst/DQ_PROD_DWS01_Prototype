...
export function MarketplaceSidebar({ collapsed, onCollapsedChange }: MarketplaceSidebarProps) {
  ...
  // Items visible in the workspace sidebar (excludes marketplace items — those are rendered separately)
  const workspaceItems = useMemo(
    () =>
      navigationItems.filter(
        (item) =>
          item.section !== 'marketplace' &&
  ...
  const isMarketplacePath = location.pathname.startsWith('/marketplaces');
  ...
  // Marketplace-specific nav items (always shown)
  const marketplaceItems = navigationItems.filter((item) => item.section === 'marketplace');

  // Sections to render in workspace portion (everything except marketplace)
  const workspaceSections = navSections.filter((s) => s.id !== 'marketplace');
  ...
  return (
    ...
    >
      <div className="flex min-h-full flex-col px-4 py-4">
        ...
        <nav aria-label="Marketplace navigation" className="flex-1 space-y-1">
          {/* ── Marketplace section (always first, always expanded) ── */}
          {(() => {
            const marketplaceSection = navSections.find((s) => s.id === 'marketplace')!;
            const isActiveSection = isMarketplacePath;
            const Icon = iconMap[marketplaceSection.icon];
              ...
              <div className="border-b border-border-subtle py-1">
                <button
                  onClick={() => {
                    ...
                    if (!isMarketplacePath) navigate('/marketplaces/services');
                  }}
                ...
                >
                  ...
                  {!collapsed && (
                    <span className="flex-1 truncate text-left">{marketplaceSection.label}</span>
                  )}
                  {!collapsed && (
                ...
                <div key={section.id} className="border-b border-border-subtle py-1 last:border-b-0">
                  ...
                  >
                    ...
                    {!collapsed && (
                      <button
                        onClick={() =>
                          setExpandedSection((cur) => (cur === section.id ? '' : section.id))
                        }
