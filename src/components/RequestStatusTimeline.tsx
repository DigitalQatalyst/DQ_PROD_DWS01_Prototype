  ...
  return (
    <div className="bg-white rounded-card border border-border-default p-6">
      ...
      <div className="relative pl-4 sm:pl-0">
        <div className="absolute left-8 top-4 bottom-4 w-px bg-border-strong hidden sm:block" />
          ...
          {timeline.map((event, i) => {
            ...
            return (
              <div key={i} className="relative flex flex-col sm:flex-row gap-4 sm:gap-8 group">
                ...
                {/* Content */}
                <div className="flex-1 bg-surface/30 rounded-md border border-border-subtle hover:border-border-strong transition-colors cursor-pointer" onClick={() => toggleEvent(i)}>
                  ...
                  {isExpanded && (
                    <div className="px-4 pb-4 pt-1 text-sm text-text-secondary leading-relaxed border-t border-border-subtle/50 mt-2">
                      {event.description}
                    </div>
                  )}
                </div>
